...
public class CityActivity extends SwipeBackActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.he_frist3_city);
    ...
    private void initData() {
        mTitle.setText(getString(R.string.frist3_curCity)+ WeatherPresenter.cityName);

        mAdapter = new CityAdapter(this, cityList);
        mListView.setAdapter(mAdapter);
        mListView.setOnScrollListener(mAdapter);
        mListView.setPinnedHeaderView(LayoutInflater.from(this).inflate(R.layout.he_frist3_city_item_section, mListView, false));
        mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {

            @Override
            public void onItemClick(AdapterView<?> parent, View view,
                                    int position, long id) {
                mTitle.setText(getString(R.string.frist3_curCity)+ mPresenter.getCityList().get(position).getName());
